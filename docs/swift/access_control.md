...
访问控制可以限制其他源文件和模块中的代码访问你的代码部分。这个功能使你能够隐藏代码的实现细节，并指定一个首选的接口，通过这个接口可以访问和使用该代码。

你可以为单独的类型（如类、结构体和枚举）指定特定的访问级别，也可以为这些类型的属性、方法、初始化器和下标指定访问级别。协议可以限制在特定的上下文中使用，全局常量、变量和函数也是如此。

Swift 不仅提供了多种不同的访问级别，还为某些典型场景提供了默认的访问级别，这样就不需要我们在每段代码中都显式声明访问级别。如果你只是开发一个单目标（`single-target`）的应用程序，完全可以不用显式声明代码的访问级别。

## 模块和源文件

Swift的访问控制模型基于模块和源文件的概念。

一个模块是代码分发的单一单位——一个作为单一单位构建和发布的框架或应用程序，可以通过Swift的`import`关键字被其他模块导入。

在Xcode中，每一个构建目标（如应用程序包或框架）被视为Swift中的一个独立模块。如果你将应用程序代码的某些部分组织成一个独立的框架——可能是为了封装并在多个应用程序中重用这些代码——那么当这个框架被导入并在一个应用内使用，或在另一个框架内使用时，框架内定义的所有内容都将成为一个独立的模块的一部分。

源文件是模块内的一个单一Swift源代码文件（实际上是应用或框架内的一个文件）。虽然通常在不同的源文件中定义不同的类型，但一个单一的源文件可以包含多个类型、函数等的定义。

## 访问级别

Swift为代码中的实体提供了五种不同的访问级别。这些访问级别相对于定义实体的源文件以及该源文件所属的模块。

- **开放访问（Open）** 和 **公开访问（Public）** 允许实体在其定义模块的任何源文件中使用，以及在导入该定义模块的其他模块的源文件中使用。当你定义一个框架的公共接口时，通常会使用开放或公开访问。开放访问与公开访问的区别在于，开放访问允许外部模块的代码对类进行子类化和覆盖。
- **内部访问（Internal）** 允许实体在其定义模块的任何源文件中使用，但不能在该模块外的源文件中使用。你通常在定义应用程序或框架的内部结构时使用内部访问。
- **文件私有访问（File-private）** 将实体的使用限制在其自身定义的源文件中。使用文件私有访问来隐藏一个特定功能的实现细节，当这些细节在整个文件中使用时。
- **私有访问（Private）** 将实体的使用限制在其声明范围内，以及同一文件中该声明的扩展中。使用私有访问来隐藏一个特定功能的实现细节，当这些细节仅在单个声明中使用时。

在这五个访问级别中，开放访问是最高（限制最少）的访问级别，私有访问是最低（限制最多）的访问级别。开放访问只适用于类和类成员，它与公开访问的区别在于允许模块外的代码对类进行子类化和覆盖，这在后文中的“子类化”部分有所讨论。将一个类标记为开放，明确表明你已考虑了来自其他模块的代码使用该类作为超类的影响，并且你已相应地设计了你的类代码。
...
当你编写一个 `single-target`应用程序时，应用的所有功能都是为该应用服务，而不需要提供给其他应用或者模块使用，所以你不需要明确设置访问级别，使用默认的访问级别 `internal` 即可。但是，你也可以使用 `fileprivate` 或 `private` 访问级别，用于隐藏一些功能的实现细节。
...
当你的应用程序包含 `Unit Test Target` 时，为了测试，测试模块需要访问应用程序模块中的代码。默认情况下只有 `open` 或 `public` 级别的实体才可以被其他模块访问。然而，如果在导入应用程序模块的语句前使用 ``@testable `特性，然后在允许测试的编译设置（`Build Options -> Enable Testability``）下编译这个应用程序模块，单元测试目标就可以访问应用程序模块中所有内部级别的实体。
...
除非专门指定，否则实体默认的访问级别为 `internal`。这意味着在不使用修饰符显式声明访问级别的情况下，`SomeInternalClass` 和 `someInternalConstant` 的访问级别是 `internal`：
